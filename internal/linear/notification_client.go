...
func (nc *NotificationClient) GetNotifications(includeRead bool, limit int) ([]Notification, error) {
	...
	if limit <= 0 {
		return nil, &ValidationError{Field: "limit", Value: limit, Reason: "must be positive"}
	}
	if limit > 1000 {
		return nil, &ValidationError{Field: "limit", Value: limit, Reason: "cannot exceed 1000"}
	}
...
func (nc *NotificationClient) MarkNotificationRead(notificationID string) error {
	...
	if notificationID == "" {
		return &ValidationError{Field: "notificationID", Message: "notificationID cannot be empty"}
	}
