...
func NewAttachmentClient(base *BaseClient) *AttachmentClient {
	...
	return &AttachmentClient{
		base:       base,
		httpClient: NewOptimizedHTTPClient(), // Reuse the optimized HTTP client
		cache:      cache,
	}
}
...
func (ac *AttachmentClient) GetAttachment(attachmentURL string, format AttachmentFormat) (*AttachmentResponse, error) {
	// Validate input
	if attachmentURL == "" {
		return nil, ValidationErrorWithExample("attachmentId", "cannot be empty", 
...
func (ac *AttachmentClient) getAttachmentMetadata(attachmentID string) (*Attachment, error) {
...
func (ac *AttachmentClient) downloadAttachment(url string) ([]byte, string, int64, error) {
...
func (ac *AttachmentClient) downloadAttachmentWithRetry(url string, maxRetries int) ([]byte, string, int64, error) {
...
func (ac *AttachmentClient) attemptDownload(url string) ([]byte, string, int64, error) {
...
func (ac *AttachmentClient) isRetryableError(err error) bool {
	if err == nil {
		return false
	}
...
func (ac *AttachmentClient) isImageContent(contentType string) bool {
	return strings.HasPrefix(contentType, "image/")
}

// getImageDimensions returns the width and height of an image
func (ac *AttachmentClient) getImageDimensions(imageData []byte) (width, height int, err error) {
...
func (ac *AttachmentClient) resizeImageForMCP(imageData []byte, contentType string) ([]byte, error) {
	// Decode the image
...
func (ac *AttachmentClient) generateCacheKey(url string, format AttachmentFormat) string {
...
func (ac *AttachmentClient) getUploadURL(size int, filename, contentType string) (*FileUploadResponse, error) {
	const mutation = `
...
func (ac *AttachmentClient) uploadToURL(uploadURL string, content []byte, contentType string, headers map[string]string) error {
...
func (ac *AttachmentClient) readFile(filepath string) ([]byte, error) {
...
func (ac *AttachmentClient) extractFilename(filepath string) string {
	// Find last / or \
...
func (ac *AttachmentClient) detectContentType(filepath string, content []byte) string {
